...
export default function InviteCrewForm() {
  const { user } = useAuth();
  const [formData, setFormData] = useState<InviteCrewFormData>({
    email: '',
    first_name: '',
    last_name: '',
    base_city: '',
  ...
  // Crew positions - as per specifications
  const crewPositions = [
    { value: 'CPT', label: 'Captain (CPT)' },
    { value: 'FO', label: 'First Officer (FO)' },
    { value: 'FA', label: 'Flight Attendant (FA)' },
  ...
  const handleChange = (field: keyof InviteCrewFormData, value: string) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl text-gray-900">Invite Crew Member</h1>
        <p className="text-gray-600">Send an invitation to a new crew member to join the platform</p>
      </div>
      ...
      <Alert>
        ...
        <AlertDescription>
          <strong>Workflow:</strong> After sending an invitation, the user will receive an email to set their password. 
          Once they log in, they'll complete their profile setup (Identity → Passport → Certificates) and await HR validation.
        </AlertDescription>
      </Alert>
    </div>
  );
